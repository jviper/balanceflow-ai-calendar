...
export const Header: React.FC<HeaderProps> = ({ currentDate, viewMode, onSetViewMode, onPrev, onNext, onToday, onOpenSettings }) => {
    const viewModes: ViewMode[] = [ViewMode.Day, ViewMode.Week, ViewMode.Month];
    const dateFormat = viewMode === 'month' ? 'MMMM yyyy' : 'MMMM d, yyyy';

    return (
        <header className="flex items-center justify-between p-4 bg-slate-900/70 backdrop-blur-lg border-b border-slate-800/50 text-slate-200">
